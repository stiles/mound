...
echo "Starting the mound publishing process..."
echo "========================================"
...
if [[ -z "${VERSION}" ]]; then
  echo "Error: Could not determine version from mound/__init__.py" >&2
  exit 1
fi
...
[[ $REPLY =~ ^[Yy]$ ]] || { echo "Cancelled."; exit 1; }
read -p "Have tests and lint passed locally (pytest, ruff check .)? (y/n) " -n 1 -r; echo
[[ $REPLY =~ ^[Yy]$ ]] || { echo "Cancelled."; exit 1; }
...
select choice in "Publish to TestPyPI" "Publish to PyPI (Official)" "Cancel"; do
  case $choice in
    "Publish to TestPyPI")
      ...
      echo "Published to TestPyPI: https://test.pypi.org/project/mound/${VERSION}/"
      break;;
    "Publish to PyPI (Official)")
      ...
      echo "Published to PyPI: https://pypi.org/project/mound/${VERSION}/"
      break;;
    "Cancel")
