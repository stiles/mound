...
- `.github/workflows/publish.yml` uploads to PyPI when a GitHub release is published, and `PUBLISH.md` writes down the whole process. The upload used to happen on whichever machine ran the script, which meant a release depended on a token in someone's shell and on that someone remembering to run it. The workflow builds its own copy from the tagged tree rather than trusting the files attached to the release, and refuses to upload if the version it built doesn't match the tag -- shipping the wrong number under the right tag is unfixable, since PyPI won't let a version be replaced.

### Changed

- The version is one literal, in `mound/__init__.py`. `pyproject.toml` declares `dynamic = ["version"]` and hatchling reads it from there, so the number `mound --version` prints and the number on the wheel can't disagree -- which two hand-maintained copies eventually do, and only visibly after the release is spent.
- `scripts/publish.sh` cuts the release instead of performing it: it bumps the version, opens the dated changelog section, runs `ruff` and `pytest`, commits, tags, builds and creates the GitHub release with notes read out of the changelog verbatim. It no longer uploads anything. Doing it locally *and* from the release is how you get a Publish run failing with "400 File already exists" against a package that already went out, so the upload lives in exactly one place.
